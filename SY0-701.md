...
## Universal plausibility requirement for every answer choice

Every answer choice, including every incorrect distractor, must be a technically meaningful and plausible answer to **some other realistic CompTIA Security+ SY0-701 question or scenario**.

In other words, no option may be nonsense, inherently insecure without contextual justification, technically impossible, irrelevant filler, or something that a knowledgeable candidate could reject in isolation without reading the stem.

A distractor should be wrong **because it does not best satisfy the specific facts, constraints, qualifier, or objective in the current question**—not because the distractor itself is generally absurd or nonviable.

Before accepting any answer choice, independently ask:

> Could this option reasonably be the correct answer to a different, well-written Security+ question?

If the answer is no, rewrite or replace that option.

Examples of unacceptable distractors include:

- controls that obviously contradict standard security practice with no scenario-specific reason;
- technologies used for functions they cannot perform;
- actions that would never be reasonable at the stated stage of an incident;
- unrelated terminology inserted only to fill an answer slot;
- deliberately reckless or nonsensical administrative actions;
- answers that are categorically inferior in essentially every plausible Security+ scenario.

Prefer distractors that are legitimate controls, processes, technologies, governance actions, attack techniques, or response steps but lose to the keyed answer because of an explicit distinction in the stem.
...
For **every individual answer choice**, including distractors, also confirm that the option could plausibly be correct in another realistic Security+ question. Reject and rewrite any option that is nonsense, technically impossible, irrelevant filler, or categorically bad security practice independent of the current stem.

The reason an incorrect option loses must be scenario-specific.
...
4. apply the strong-competitor rules and verify universal plausibility of every answer choice;
